...
// @desc    Get user profile (storefront)
// @access  Private
router.get("/profile", protect, async (req, res) => {
  try {
    const user = await User.findById(req.user.id).select("-password").populate("vendorProfile", "slug status shopName");
    if (!user) {
...
// @desc    Update user profile (storefront)
// @access  Private
router.put("/profile", protect, async (req, res) => {
  try {
    ...
    const { name, email, phone, address, city, pincode, location, bio, image } = req.body;
    ...
    const populated = await user.populate("vendorProfile", "slug status shopName");
    const userPayload = {
      id: populated._id,
      name: populated.name,
      ...
      vendorSlug: populated.vendorProfile?.slug || null,
    };
