...
function Login() {
  ...
  useEffect(() => {
    ...
    const qUserStr = params.get("user");
    if (qToken && qUserStr) {
      try {
        const qUser = JSON.parse(decodeURIComponent(qUserStr));
        localStorage.setItem("token", decodeURIComponent(qToken));
        localStorage.setItem("user", JSON.stringify(qUser));
        redirectByRole(qUser);
      } catch (err) {
